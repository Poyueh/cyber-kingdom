extends SceneTree
## External harness: load only packaged res:// content via --main-pack from --path /tmp.
var failures:=0
var assertions:=0
...
func run_scene() -> void:
	...
	check(ResourceLoader.exists("res://art/frontier/variety-v001/arch.png") and game.view._details.size()>=36,"packaged map includes varied biome scenery")
	check(not ResourceLoader.exists("res://art/frontier/variety-v001/sources/props.png"),"raw art sheet stays outside release package")
	check(game.sim.hero.stats.attack_cost==12 and game.sim.hero.stats.jump_cost==18,"packaged knight uses new action stamina rules")
	...
	root.get_texture().get_image().save_png("/tmp/cycle-packaged-menu.png")
	game.queue_free();await process_frame
