...
func _initialize() -> void:
	...
	print(JSON.stringify({"probe":"stationary-defense-no-knight","step":1.0/60,"results":results},"  "))
	quit()
func measure(day: int, guards: int, hunters: int, wall_level: int) -> Dictionary:
	...
	sim.world.wall.merge({"level":wall_level,"hp":wall_level*40}, true)
	assert(sim.world.wall.hp == wall_level * 40, "Probe must start with a built wall")
	...
	return {"day":day,"guards":guards,"hunters":hunters,"wall_level":wall_level,
		"wall_hp":sim.world.wall.hp,"remaining_defenders":survivors,"initial_defenders":guards+hunters,
		"seconds":snappedf(elapsed,0.1),"reached_dawn":sim.clock.day>day,
		"outcome":"held" if sim.world.wall.hp>0 and survivors==guards+hunters and sim.clock.day>day else ("lost" if survivors==0 else "breached")}
