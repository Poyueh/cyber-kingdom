...
訓練場已接入四格待機、六格跑步與八格斬擊與四格衝刺。跳躍暫用中立姿勢，守衛和鑄造場也已換成第一版生成素材。這是可玩版本的第一輪素材，還需要動作打磨與手機辨識度驗證。
...
| 下劈 v002（保留） | [source/attack-v002.png](source/attack-v002.png) | [提示詞](source/attack-v002.prompt.txt) | processed/attack-v002.png |
...
新版來源：`source/attack-v003.png`、`source/dash-v001.png`，各附同名 `.prompt.txt`。處理後使用 `processed/attack-v003.png` 與 `processed/dash-v001.png`。兩者以原待機作角色參考；裁切配方的 erase_rects 僅移除跨格混入的相鄰角色碎片，保留完整劍身。

## 遊戲用規格

- 每格 128 × 96，腳底錨點 (64, 80)，角色身高約 48 像素。
- 待機圖 256 × 192；跑步圖 384 × 192；新版斬擊圖 512 × 192、衝刺圖 256 × 192（舊揮劍圖 256 × 192 保留）。
...
- 斬擊八格對應四格起手、兩格有效揮擊、兩格收招；衝刺四格對應起步、推進、低身與煞停；遊戲依戰鬥進度選格，SpriteFrames 的 attack FPS 只供編輯器預覽。
- 預設整刀 0.34 秒：前 40% 舉劍，40% 至 75% 可命中，最後 25% 收招。刀光與傷害使用相同時間窗口。
...
原圖、處理後畫格及 Godot 真正渲染結果已目視檢查；[近景動作預覽](../../../docs/previews/mobility-cleave-v003.gif) 展示實際跑步與擊敗守衛。展示用攝影機放大兩倍，專案預設鏡頭未改動。

新版下劈時長權重為 0.10、0.10、0.10、0.10、0.15、0.20、0.10、0.15。命中停頓由遊戲規則觸發，不寫入圖片或靠重複畫格模擬。原始 v001 素材與提示詞仍保留。
