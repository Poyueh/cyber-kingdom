...
func test_direction_selects_intent_on_either_side(t):
 ...
 t.equal(state.axis,-1.0,"right-side horizontal swipe moves left independently")
 state.finish(1);t.equal(state.axis,-1.0,"payment release keeps second thumb moving")
