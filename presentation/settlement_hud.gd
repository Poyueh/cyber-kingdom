...
func _ready() -> void:
	...
	$Refuge.text = "訓練場"
	$Top/Title.text = "最後的避難所 / 現場建設"
	$Keys.text = "A/D 移動　E 投入　J 劈砍　L 衝刺　Space 跳躍　R 重試"
...
func present_world(sim, paused: bool, at: float, grounded: bool) -> void:
	status.text = "生命 %d  護盾 %d  廢料 %d  龍晶 %d" % [sim.hero.hp,sim.hero.shield,sim.world.scrap,sim.world.crystals]
	var choice: Dictionary = sim.context(at)
	interact_button.text = "投入 %d %s  [E]" % [choice.cost,choice.currency] if choice.cost>0 else "互動 [E]"
	interact_button.disabled = paused or not grounded or not choice.enabled or not sim.hero.is_alive()
	if paused:
		message.text = "已暫停 / Esc 或 PAUSE 繼續"
	elif not sim.hero.is_alive():
		message.text = "騎士倒下 / R 重試整段避難所"
	elif sim.finished():
		message.text = "三波試煉結束。留下的居民與防線，就是本輪的結果。"
	elif not sim.raiders.is_empty():
		message.text = "第 %d 波來襲！保護居民；敵人倒下的廢料要靠近回收。" % sim.wave
	else:
		message.text = "夜襲約 %d 秒後 / 招攬居民 → 供應器具 → 建造防線" % ceili(sim.time_to_raid)
