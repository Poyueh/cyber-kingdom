...
const TC=preload("res://art/fonts/noto-sans-tc/NotoSansTC-Regular.otf")
const SC=preload("res://art/fonts/noto-sans-sc/NotoSansSC-Regular.otf")
...
static func current() -> Font:
 ...
 _shared.base_font=SC if simplified else TC
 _shared.fallbacks=[TC if simplified else SC]
 return _shared
