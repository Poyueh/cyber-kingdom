...
func _ready() -> void:
	...
	_label(Vector2(32,62),Vector2(896,26),15,Color("8da8b7")).text = "龍晶分配試驗  /  每輪重置，尚未保存出征進度"
	_forecast = _label(Vector2(32,99),Vector2(896,33),18,Color("e5ac7a"))
	_card(Rect2(32,145,432,163))
	_card(Rect2(488,145,440,163))
	_label(Vector2(56,158),Vector2(200,28),20,Color("90e4df")).text = "改造騎士"
	...
	var back := _button("返回訓練場",Rect2(32,453,220,52))
	back.pressed.connect(func(): exit_requested.emit())
	depart_button = _button("確認配置，出征",Rect2(616,449,312,60))
...
func present_allocation(snapshot: Dictionary) -> void:
	...
	_title.text = "最後的避難所  /  這次，能源留給誰？"
	_forecast.text = "魔潮預報：%d 名居民各承受一次衝擊。每顆龍晶可護住 1 人。" % snapshot.residents
	_knight.text = "護盾  %d\n分配 %d 顆龍晶" % [snapshot.knight_shield,snapshot.knight_crystals]
	_refuge.text = "居民防護  %d / %d 人" % [snapshot.protected,snapshot.residents]
	residents.present(snapshot,false)
	_summary.text = "共 %d 顆龍晶：騎士 %d ／ 避難所 %d。\n預計 %d 人受傷；騎士護盾承受傷害後不再恢復。" % [snapshot.total,snapshot.knight_crystals,snapshot.refuge_crystals,snapshot.wounded]
	_caption.text = "擊敗守衛帶回廢料；撤退或倒下也會結算魔潮。出征後配置鎖定。"
	depart_button.text = "確認配置，出征"
	if _choice_count != snapshot.total + 1:
		...
		for amount in range(_choice_count):
			var button := _button("騎士 %d  /  避難所 %d" % [amount,snapshot.total-amount],Rect2(32+amount*(width+8),322,width,58))
			if _choice_count > 4:
...
func present_result(report: Dictionary) -> void:
	var outcome: String = {"victory":"出征成功", "retreat":"提前撤退", "defeat":"騎士倒下"}[report.outcome]
	_title.text = "魔潮過後  /  " + outcome
	_forecast.text = "魔潮已結算：%d 人安然無恙，%d 人受傷。" % [report.protected,report.wounded]
	_knight.text = "生命  %d\n護盾擋下 %d 傷害" % [report.hero_hp,report.absorbed]
	_refuge.text = "避難所  /  %d 名居民受傷" % report.wounded
	residents.present(report,true)
	for button in choices:
		button.hide()
	_summary.position.y = 333
	_summary.text = "帶回廢料 %d　剩餘護盾 %d\n本次配置：騎士 %d 顆 ／ 避難所 %d 顆" % [report.recovered,report.shield_remaining,report.knight_crystals,report.refuge_crystals]
	_caption.text = "重試會還原本輪龍晶與居民，供比較不同選擇；這不是永久死亡或完整經營系統。"
	depart_button.text = "重新分配，再試一次"
