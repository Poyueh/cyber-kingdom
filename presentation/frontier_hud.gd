...
func _ready() -> void:
	...
	$Keys.text = "A/D 移動　E 標記／建設　J 戰鬥　L 衝刺　Space 跳躍　R 重玩此圖"
...
func present_world(sim, is_paused: bool, at: float, grounded: bool) -> void:
	...
	$Top/Title.text = "龍晶邊境 #%d · 探索 %d/6 · 拓荒站 %d" % [sim.map_seed,map.discovered_count(),map.outpost_count()]
	...
	if sim.kingdom_established():
		...
		message.text = "夜襲 %d 秒 / 城鎮 %d/3 · 工坊備器具 → E 標記 → 工匠採集搬運" % [ceili(sim.time_to_raid),map.city_level]
	var choice: Dictionary = sim.context(at)
	if choice.id == "mark": interact_button.text = "標記居民工作 [E]"
