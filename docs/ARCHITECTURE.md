...
Combatant 持有整刀進度：前 40% 舉劍、40% 至 75% 可命中、最後 25% 收招。傷害窗口、揮劍畫格與刀光均讀取這份進度；呈現層不另設傷害計時器。起手時保存出劍朝向，途中反向移動不會把這一刀翻到背後；衝刺取消當前揮劍。單刀模式即使冷卻比整刀短，也必須收招完畢才能再開始；騎士連斬使用下述明確接招窗口。
...
knight_combo.tres 由 bootstrap 的 Mapper 轉成純 Stats，訓練、出征與主戰役都用相同騎士設定。第二刀以反向「時間」選取現有攻擊圖，保留各格權重；下劈結束的低劍姿勢接回挑，回挑的高劍姿勢接第三刀。MovingAttack 圖集共用同一選格，腿部維持連續步伐。
