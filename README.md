...
目前版本包含**戰鬥訓練場與可走動避難所原型**，已有騎士待機、跑步、八格斬擊、四格衝刺及可連續通過的平台路線、鐵甲守衛與古城鑄造場背景，尚未包含完整 Roguelite、持續經濟或完整動作美術。
...
F5 進入遊戲後，點下方 **REFUGE / 避難所**。也可在編輯器開啟 `scenes/settlement.tscn` 按 F6。

1. 靠近流浪者，按 **E** 或畫面「投入」，花 1 廢料投放補給。對方拾取後加入。
2. 向右走到工坊、武器坊，各花 2 廢料供應器具；居民會自己來拿，成為工程師或守備兵。
3. 再向右走到防線，花 3 廢料下建造單，等工程師施工。完成後可再投入升級。
4. 夜襲從右側接近；你可按 J／L 交戰，也可依靠守備兵和防線。只有未被擋住的實際命中會使居民退回流浪者。
5. 擊敗敵人後靠近拾取廢料，再補器具、修復、重新招攬。R 重試整段原型，Esc 暫停。

義肢爐與護民塔共用有限龍晶：給騎士護盾，或擋住居民的下一次攻擊。第一版有三波試煉，沒有永久進度，原本訓練場存檔不受影響。建築與居民先用示意美術，角色動作細修暫緩。

[自主施工預覽](docs/previews/settlement-construction-v001.gif) · [玩法規則與費用](docs/design/walkable-refuge.md)。舊的選單分配場景 `scenes/refuge.tscn` 保留作比較，已不是主入口。
...
- [第七課：調整第一次夜襲的準備時間](docs/lessons/07-walkable-refuge.md)
- [第六課：修改每顆龍晶的護盾量（舊原型）](docs/lessons/06-dragon-crystal-allocation.md)
